...
class PatchExpand(nn.Module):
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        """
        Args:
            dim: int, number of input channels in the last dimension
        """
        super().__init__()
        self.dim = dim
...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x, feat, skip_connection="add", resized_window_size=None):

        """Forward function.
            
        Args:
            x: Input feature, tensor size (B, C, D, H, W).
            feat: feature in the same level, tensor size (B, C, D, H, W).
            skip_connection: str, "add" or "cat", default: "add"
        Returns:
            x: Output feature, tensor size (B, C, D, H, W)
        ...
        x = rearrange(x, "b c d h w -> b d h w c")
        feat = rearrange(feat, "b c d h w -> b d h w c")
...
class SwinUnet3D(nn.Module):
    ...
    def forward_down(self, x, multi=False, layer=-1, adaptive_window_size=False):
    ...
    def forward_up(self, x, feats, adaptive_window_size=False):
        if adaptive_window_size:
            resized_window_size = get_adaptive_window_size(
                self.window_size, x.shape[2:], self.base_x_size
            )
        ...
        for l, mlayer in enumerate(self.uplayers):
            x = mlayer(x, feats[-l-1], skip_connection="add", resized_window_size=resized_window_size)

        return x
    
    def forward(self, x, multi=False, layer=-1, adaptive_window_size=False):
            
        x, feats = self.forward_down(x, multi, layer, adaptive_window_size)
        x = self.forward_up(x, feats)

        return x

    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(SwinUnet3D, self).train(mode)
        self._freeze_stages()
